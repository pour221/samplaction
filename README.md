...
Samplaction helps generate a homogeneous subset of assemblies suitable for downstream comparative genomics analyses.
## Synopsis
...
Therefore, the first step in many studies is filtering assemblies to create a homogeneous dataset. Samplaction's aim is to automate this step based on assembly statistics (`seqkit stats`). Using those stats it performs:
- heuristic QC filtering
- PCA dimensionality reduction
- DBSCAN clustering for outlier detection

## Filtering strategy
Samplaction performs sample filtering in three consecutive stages.
...
First, obvious outliers are removed using simple heuristic rules. Assemblies are filtered if they violate any of the following conditions:
...
External tool:
- `seqkit`
or a table generated by `seqkit stats -a`
...
```
- Run on precomputed quality table with indication of the genome size:
```commandline
